(() => {
  ...
  function handleResize() {
    if (window.innerWidth > 991) (() => {
      const modules_flsModules = {};
      class WatchOnceSections {
        constructor(props) {
          let defaultConfig = {
            logging: true
          };
          this.config = Object.assign(defaultConfig, props);
          this.sections = document.querySelectorAll(".veterinarians");
          this.observer;
          this.isFirstPass = true;
          !document.documentElement.classList.contains("watchonce") ? this.watchOnceRun() : null;
        }
        watchOnceRun() {
          document.documentElement.classList.add("watchonce");
          this.observeSectionsInit();
        }
        watchOnceSectionsUpdate() {
          this.observeSectionsInit();
        }
        observeSectionsInit() {
          this.observer = new IntersectionObserver(((entries, observer) => {
            entries.forEach((entry => {
              this.observeSectionsCallback(entry, observer);
            }));
          }), {
            threshold: .4
          });
        ...
        observeSectionsCallback(entry, observer) {
          const targetElement = entry.target;
          this.isFirstPass = true;
          this.sectionIntersecting(entry, targetElement);
          entry.isIntersecting ? this.sectionOff(targetElement, observer) : null;
          if (entry.intersectionRatio > .4 && this.isFirstPass) {
            const sectionHeight = entry.target.offsetHeight;
            const middleOffset = sectionHeight * .44;
            const scrollPosition = window.pageYOffset || document.documentElement.scrollTop;
            const scrollToCenter = scrollPosition + middleOffset;
            window.scrollTo({
              top: scrollToCenter,
              behavior: "smooth"
            });
            ...
            setTimeout((() => {
              ...
              const scrollDistance = window.innerHeight * .033;
              window.scrollBy(0, scrollDistance);
            }), 3e3);
            this.isFirstPass = false;
            document.dispatchEvent(new CustomEvent("sectionsCallback", {
              detail: {
                entry
              }
            }));
        ...
        sectionOff(targetElement, observer) {
          observer.unobserve(targetElement);
        }
        sectionIntersecting(entry, targetElement) {
          if (entry.isIntersecting);
        }
      }
      modules_flsModules.watchonce = new WatchOnceSections({});
    })();
  }
